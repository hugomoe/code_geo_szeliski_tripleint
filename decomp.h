...
printf("mu1 x nu1 : %d x %d\n",mu1,nu1);
	...
	//On calcule la positions des coins des images intermediaire (x2,y2 puis x3,y3)
	...
	double det_A = A[0]*A[4]-A[3]*A[1];
        ...
        //x2,y2 = AA(x1,y1) : img2 sera l'image issue d'une rotation-translation de l'image d'entrée (img2 = A^(-1)(img1))
        double x2[4];
        double y2[4];
        for(int k=0;k<4;k++){
        ...
        //on encadre ces 4 coins dans un rectangle d'origine (mu2,nu2) et de taille w2*h2
...
printf("mu2 x nu2 : %d x %d\n",mu2,nu2);
        ...
        //x3,y3 = B(x4,y4) : img3 sera l'image qui, par une rotation, sera transformée en l'image de sortie (img4 = B^(-1)(img3))
        double x3[4];
        double y3[4];
        for(int k=0;k<4;k++){
        ...
        //on encadre ces 4 coins dans un rectangle d'origine (mu3,nu3) et de taille w3*h3
...
printf("mu3 x nu3 : %d x %d\n",mu3,nu3);
	...
	//la dernière image intermédiaire sera l'image de sortie
        int w4 = w_f, h4 = h_f;
        int mu4 = 0, nu4 = 0;
...
printf("mu4 x nu4 : %d x %d\n\n",mu4,nu4);
        ...
        //à ce stade, on a pris en compte une grande partie des translations, via les mu,nu
...
printf("application de B  : done\n");
	...
	//on tronque la zone due a la symetrisation (dans "affine.h", pour éviter l'effet gibbs)
	double p[2];
	for(int i=0;i<w_f;i++){
		for(int j=0;j<h_f;j++){
			p[0]=i; p[1]=j;
